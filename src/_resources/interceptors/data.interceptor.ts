...
export default (chain: any) => {
  ...
  return chain
    ...
    .then((res: { statusCode: number; data: any }) => {
      ...
      // data 返回数据
      resultData.code = resultData.data.hasOwnProperty('code')
        ? resultData.data.code.toString()
        : res.statusCode.toString()

      resultData.success = SUCC_LIST.includes(resultData.code)
      ...
      if (LOGIN_FAILURE_LIST.includes(resultData.code)) {
        ...
        // if (Page.getCurRouter() === Page.getRouters().home) {
        //   setTimeout(() => {
        //     Taro.navigateTo({
        //       url: `/${Page.getRouters().auth}?from=home`,
        //     })
        //   }, 800)
        // } else {
        //   setTimeout(() => {
        //     Taro.navigateTo({
        //       url: `/${Page.getRouters().auth}`,
        //     })
        //   }, 800)
        // }
